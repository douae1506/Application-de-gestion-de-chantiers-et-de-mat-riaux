    ...
    <div class="auth-card">
      ...
      <form @submit.prevent="handleLogin" novalidate>
        ...
        <div class="field" :class="{ error: errors.email }">
          ...
          <input id="email" v-model="form.email" type="email" placeholder="nom@entreprise.com" autocomplete="email" :disabled="loading" />
        ...
        <div class="field" :class="{ error: errors.password }">
          ...
          <div class="input-wrapper">
            <input id="password" v-model="form.password" :type="showPassword ? 'text' : 'password'" placeholder="••••••••" autocomplete="current-password" :disabled="loading" />
...
async function handleLogin() {
  errors.value = {}; errorMessage.value = ''; loading.value = true
  try {
    const user = await auth.login(form.email, form.password)
    router.push(user.redirect_to)
  } catch (err) {
    if (err.errors) errors.value = err.errors
    else errorMessage.value = err.message ?? 'Une erreur de connexion est survenue.'
...
.auth-card { background:#fff; border-radius:20px; padding:3rem 2.5rem; width:100%; max-width:440px; border:1px solid #e2e8f0; box-shadow:0 20px 25px -5px rgba(0,0,0,.03),0 10px 10px -5px rgba(0,0,0,.02); }
...
.label-row { display:flex; align-items:center; justify-content:space-between; margin-bottom:.5rem; }
.label-row label { margin-bottom:0; }
.forgot-link { font-size:.8rem; font-weight:600; color:#2563eb; text-decoration:none; transition:color .15s; }
...
@media (max-width:480px) { .auth-layout { background:#fff; align-items:flex-start; padding-top:3rem; } .auth-card { border:none; box-shadow:none; padding:1rem; } }
</style>
